...
class IndexValidationTests(unittest.TestCase):
    ...
    def test_reports_missing_metric_column(self):
        frame = self.frame("overall_index")
    ...
    def test_reports_missing_base_columns(self):
        frame = pd.DataFrame({"overall_index": [0.5]})
    ...
    def test_rejects_unknown_metric(self):
        with self.assertRaisesRegex(DataValidationError, "Unknown metric"):
            validate_index_data(self.frame("overall_index"), "made_up_index")
...
class DaySectionValidationTests(unittest.TestCase):
    """Page 2's schema: h3_id/segment/hour_bucket/Week_part/<metric>."""
    ...
    def frame(
        self, metric, values=None, segments=None, buckets=None, week_parts=None
    ):
        values = [0.5, 0.25] if values is None else values
        segments = ["Families", "Families"] if segments is None else segments
        buckets = ["Morning", "Noon"] if buckets is None else buckets
        week_parts = (
            ["Weekday"] * len(values) if week_parts is None else week_parts
        )
        return pd.DataFrame(
            {
                "h3_id": [self.cell] * len(values),
    ...
    def test_reports_missing_week_part_column(self):
        """The live day-section tables carry Week_part, so the local CSV
        fallback has to as well - otherwise the Weekday/Weekend radio would
        have nothing to filter on."""
        frame = self.frame("overall_index").drop(columns=[WEEK_PART_COLUMN])
        with self.assertRaisesRegex(DataValidationError, WEEK_PART_COLUMN):
            validate_day_section_data(frame, "overall_index")

    def test_rejects_blank_week_part(self):
        frame = self.frame(
            "volume_index",
            values=[0.5],
            segments=["Families"],
    ...
    def test_week_part_survives_validation(self):
        result = validate_day_section_data(
            self.frame("overall_index", week_parts=["Weekday", "Weekend"]),
            "overall_index",
        )
        self.assertEqual(
            sorted(result.data[WEEK_PART_COLUMN].tolist()),
            ["Weekday", "Weekend"],
        )
...
class DaySectionAggregationTests(unittest.TestCase):
        ...
        frame = pd.DataFrame(
            {
                ...
                WEEK_PART_COLUMN: ["Weekday", "Weekday"],
                metric: [0.2, 0.8],
            }
        )
        self.assertEqual(len(collapse_day_section_duplicates(frame, metric)), 2)

    def test_collapse_keeps_week_parts_separate(self):
        """Weekday and Weekend are different rows of the same day-part."""
        metric = "overall_index"
        frame = pd.DataFrame(
            {
                "h3_id": [self.cell, self.cell],
                "segment": ["Families", "Families"],
                "hour_bucket": ["Morning", "Morning"],
                WEEK_PART_COLUMN: ["Weekday", "Weekend"],
                metric: [0.2, 0.8],
            }
        )
        self.assertEqual(len(collapse_day_section_duplicates(frame, metric)), 2)
        ...
        frame = pd.DataFrame(
            {
                ...
                WEEK_PART_COLUMN: ["Weekday"] * 4,
                metric: [0.2, 0.6, 999.0, 0.1],
            }
        )
        result = aggregate_day_section_cells(
            frame, ["Families", "HNWI"], "Morning", "Weekday", metric
        )
        self.assertEqual(len(result), 2)
        self.assertAlmostEqual(
            result.loc[result["h3_id"] == self.cell, metric].iloc[0], 0.4
    ...
    def test_aggregate_applies_day_part_and_week_part_together(self):
        """The two filters compose: only the Weekday Morning row survives, so
        neither dimension can silently average the other's slices."""
        metric = "overall_index"
        frame = pd.DataFrame(
            {
                "h3_id": [self.cell] * 4,
                "segment": ["Families"] * 4,
                "hour_bucket": ["Morning", "Morning", "Night", "Night"],
                WEEK_PART_COLUMN: [
                    "Weekday",
                    "Weekend",
                    "Weekday",
                ...
                metric: [1.0, 2.0, 4.0, 8.0],
            }
        )
        for day_part, week_part, expected in (
            ("Morning", "Weekday", 1.0),
            ("Morning", "Weekend", 2.0),
            ("Night", "Weekday", 4.0),
            ("Night", "Weekend", 8.0),
        ):
            with self.subTest(day_part=day_part, week_part=week_part):
                result = aggregate_day_section_cells(
                    frame, ["Families"], day_part, week_part, metric
                )
                self.assertEqual(len(result), 1)
                self.assertAlmostEqual(result.loc[0, metric], expected)
        ...
        frame = pd.DataFrame(
            {
                ...
                "segment": ["Families"],
                "hour_bucket": ["Morning"],
                WEEK_PART_COLUMN: ["Weekday"],
                metric: [0.5],
            }
        )
        result = aggregate_day_section_cells(
            frame, ["Families"], "Night", "Weekday", metric
        )
        self.assertTrue(result.empty)

    def test_aggregate_returns_empty_for_unmatched_week_part(self):
        metric = "overall_index"
        frame = pd.DataFrame(
            {
                "h3_id": [self.cell],
        ...
        result = aggregate_day_section_cells(
            frame, ["Families"], "Morning", "Weekend", metric
        )
        self.assertTrue(result.empty)

    def test_aggregate_uses_only_the_one_selected_segment(self):
        """The sidebar offers a single segment, so a one-element list must
        select that segment alone rather than blending its neighbours in."""
        metric = "overall_index"
        frame = pd.DataFrame(
            {
                "h3_id": [self.cell, self.cell],
        ...
        result = aggregate_day_section_cells(
            frame, ["HNWI"], "Morning", "Weekday", metric
        )
        self.assertEqual(len(result), 1)
        self.assertAlmostEqual(result.loc[0, metric], 0.8)
...
class ColorScaleTests(unittest.TestCase):
    ...
    def test_shared_hue_does_not_merge_the_metric_scales(self):
        ...
        # The ramp itself is shared, so the difference is the scale alone.
        self.assertIs(
    ...
    def test_all_null_values_stay_mid_ramp(self):
        position = normalize(pd.Series([np.nan, np.nan]), "overall_index")
        self.assertTrue((position == 0.5).all())
    ...
    def test_colors_track_magnitude(self):
        ...
        low, high = colors_for(values, "overall_index", dark_basemap=False)[:: 19]
    ...
    def test_format_value_matches_the_scale(self):
        self.assertEqual(format_value(0.5, "overall_index"), "0.500")
    ...
    def test_legend_reports_both_ends(self):
        html = legend_html("overall_index", 0.1, 0.9, dark_basemap=False)
