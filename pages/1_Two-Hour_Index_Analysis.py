...
``<metric>`` / ``hour_bucket``, where ``hour_bucket`` is the INT64 **two-hour
period** of the day (0, 2, ... 22 on the live tables). The page shows exactly
one period at a time, chosen with the two-hour slicer.
...
every selected segment weighs the same regardless of how many rows it brings.

Production reads BigQuery. A local CSV (uploaded, or the committed synthetic
``data/sample_index_two_hours.csv`` that carries all three metric columns and
...
def load_bigquery_cells(
    table_fqn: str, metric: str, segments: tuple, two_hour_period: int
) -> pd.DataFrame:
    sql, params = build_index_query(
        table_fqn, metric, list(segments), two_hour_period
    )
...
def bigquery_frame(metric: str) -> tuple:
    ...
    try:
        raw = load_bigquery_cells(
            table_fqn, metric, tuple(selected_segments), selected_period
        )
    except BigQueryCredentialsError as error:
        _credentials_error(error)
...
def local_frame(metric: str) -> tuple:
    ...
    aggregated = aggregate_two_hour_index_cells(
        data, selected_segments, selected_period, metric
    )
    return aggregated, selected_segments, selected_period, source_name
...
try:
    if source_mode == SOURCE_BIGQUERY:
        cells, selected_segments, selected_period, source_name = bigquery_frame(metric)
    else:
        cells, selected_segments, selected_period, source_name = local_frame(metric)
...
if cells.empty:
    st.info("No H3 cells match the selected segments and two-hour period.")
...
st.caption(
    ...
    f"the {period_label(selected_period)} period. Data source: {source_name}."
)
